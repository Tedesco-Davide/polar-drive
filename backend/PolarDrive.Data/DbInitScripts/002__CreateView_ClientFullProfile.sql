...
PhoneNumbers AS (
    ...
    LEFT JOIN PhoneVehicleMappings pvm ON vs.VehicleId = pvm.VehicleId
...
FROM CompanyStats cs
LEFT JOIN VehicleStats vs ON cs.CompanyId = vs.ClientCompanyId
LEFT JOIN PhoneNumbers pn ON cs.CompanyId = pn.CompanyId
ORDER BY cs.Name, vs.Brand, vs.Model;