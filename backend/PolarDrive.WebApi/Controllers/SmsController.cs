    ...
    ISmsConfigurationService smsConfig) : ControllerBase
{
    ...
    private async Task<ActionResult> HandleAdaptiveProfileCommand(SmsWebhookDTO dto, SmsAuditLog auditLog, string command, string from)
    {
        ...
        if (profileEvent == null)
        {
            ...
            profileEvent = new SmsAdaptiveProfile
            {
                ...
                AdaptiveSurnameName = fullName,
                ReceivedAt = DateTime.Now,
                ExpiresAt = DateTime.Now.AddHours(AppConfig.SMS_ADAPTIVE_HOURS_THRESHOLD),
        ...
        else
        {
            ...
            profileEvent.ExpiresAt = DateTime.Now.AddHours(AppConfig.SMS_ADAPTIVE_HOURS_THRESHOLD);
        ...
        Brand: {vehicle.Brand} 
        VIN: {vehicle.Vin} 
        Validità profilo: {AppConfig.SMS_ADAPTIVE_HOURS_THRESHOLD} ore";
    ...
    public ActionResult GetAvailableBrands()
    {
        try
        {
            ...
            return Ok(new { brands, polarDriveMobileNumber = AppConfig.SMS_ADAPTIVE_MOBILE_NUMBER });
        }
        catch (Exception ex)
        {
    ...
    public async Task<ActionResult> GetAdaptiveProfileStats(int vehicleId)
    {
        try
        {
            ...
            return Ok(new
            {
                ...
                totalHoursApprox = totalSessions * AppConfig.SMS_ADAPTIVE_HOURS_THRESHOLD
            });
        }
        catch (Exception ex)
        {
