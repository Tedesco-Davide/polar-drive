...
public class TeslaFakeDataReceiverController(PolarDriveDbContext db, IWebHostEnvironment env, IServiceProvider serviceProvider) : ControllerBase
{
    ...
    public async Task<IActionResult> ReceiveVehicleData(string vin, [FromBody] JsonElement data)
    {
        ...
        try
        {
            ...
            if (!vehicle.Brand.Equals(VehicleConstants.VehicleBrand.TESLA, StringComparison.CurrentCultureIgnoreCase))
            {
    ...
    public async Task<IActionResult> GetGlobalStats()
    {
        try
        {
            ...
            var teslaVehicles = await _db.ClientVehicles.CountAsync(v => v.Brand.ToLower() == VehicleConstants.VehicleBrand.TESLA);
