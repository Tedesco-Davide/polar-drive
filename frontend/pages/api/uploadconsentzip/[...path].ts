...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const fullUrl = `${apiUrl}/api/UploadConsentZip/${backendPath}${queryString ? `?${queryString}` : ''}`;

  try {
    // Handle POST requests (upload ZIP to existing consent)
    if (req.method === 'POST') {
    ...
    console.error(`[API Route] UploadConsentZip/${backendPath} error:`, error);
